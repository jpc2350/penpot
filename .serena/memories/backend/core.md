...
- `app.rpc.commands.*`: RPC command implementations exposed under `/api/rpc/command/<cmd-name>`. Read this when adding or modifying an RPC endpoint.
- `app.rpc.permissions`: permission predicate/check helper factories. Read this when changing who can access an object.
- `app.http.*`: HTTP routes and middleware. Read this when modifying request/response handling, CORS, or shared-key auth.
- `app.auth.*`: provider-specific authentication helpers such as LDAP/OIDC. Read this when changing login/registration flows.
- `app.loggers.*`: audit, webhook, database, and external log integrations. Read this when adding observability or changing event emission.
- `app.db.*` / `app.db`: next.jdbc wrapper and SQL helpers. Read this when writing queries or changing persistence patterns.
- `app.tasks.*`: background task handlers. Read this when adding or modifying an async job.
- `app.worker`: task execution/cron plumbing. Read this when changing dispatch, retry, or scheduling internals.
- `app.main`: Integrant system map and component wiring. Read this when adding a new component or changing system startup order.
- `app.config`: `PENPOT_*` env config and feature flags. Read this when introducing a new env var or feature gate.
- `app.srepl.*`: development REPL helpers for manual backend operations (data inspection, migration helpers, one-off admin tasks). Read this when you need to interactively inspect or mutate backend state.
- `app.nitrate`, `app.rpc.commands.nitrate`, and `app.rpc.management.nitrate`: external Nitrate subscription/organization integration, gated by the `:nitrate` feature flag and shared-key HTTP calls. Read this when working on Nitrate-specific features.
